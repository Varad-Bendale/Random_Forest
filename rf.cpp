...
void perfect_variable(const vector<vector<pair<float , pair<float,float>>>>& data_new);
...
vector<float>smallest_gini_imp(7) ; 
vector<pair<float  , float>> pr ; 
vector<pair<float  , float>> pos ; 
vector<float> final_ans ; 
vector<float > hash_left ;
vector< float > hash_right ; 
...
void perfect_variable(  unordered_map<float , pair<float , float>>leaf_info ,  vector<vector<pair<float , pair<float,float>>>> & data_new){
    ...
    for ( int i = 0 ; i< n-1 ; i++){
        ...
        for (int j = 0  ; j < data_new[i].size() ; j++ ){
            if ( used_variables[data_new[i][j].first  ] == 0 ){
...
vector<vector<pair<float , pair<float,float>>>> sorted_dataset(const vector<vector<float>> & data  ){
    ...
    for (int i = 0 ;  i< m ; i++){
        vector<pair<float,pair<float,float>>>temp_data  ; 
        for (int j = 0 ; j < n ; j++ ){
            num = data[j][i] ; 
            target = data[j][m-1] ; 
            index = (float)j ; 
            temp_data.push_back({num , {index , target} }) ; 
        }
        data_info.push_back(temp_data) ; 
    }
    for ( int i = 0 ; i < n-1  ; i++ ){ 
        vector<pair<float,pair<float,float>>>temp_data  ; 
        for ( int j = 0 ; j < m ; j++ ){
            temp_data.push_back(data_info[i][j]) ; 
        }
        sort(temp_data.begin() , temp_data.end()) ; 
        for ( int k = 0  ; k < m ; k++ ){
            data_info[i][k] = temp_data[k] ;  
        }
    }
...
vector<float> random_forest(){
    ...
    for (int i = 0 ; i < data.size() ; i++){
        ...
        vector<float>answer = answ(tree.first , tree.second , test_data) ;
        #pragma omp critical
        {
           pred_ans.push_back(answer);
        }
        delete tree.first;   
        delete tree.second;
    }
